...
* **Sparse Section Bitmasking (SSTI)**  
  Skip empty sections in Moonrise's outer random-tick loop. Dense chunks keep the linear scan. Same RNG and section order. The mask is built on load, updated when a section crosses 0/non-zero ticking blocks, and rebuilt if a plugin replaces a section object through `getSections()` (WorldEdit/FAWE). Isolated `optimiseRandomTick` time, not whole-server MSPT: normal overworld unchanged; skyblock 231 to 121 µs; End 234 to 115 µs. Numbers and reproduce steps: [docs/TEST.md](docs/TEST.md).
